/**
 * Ivelt Topic Hider - Content Script
 * Version: 1.0.1
 */
...
function init(hiddenIds) {
  ...
  const { hiddenListElement, statsBar, toggleBtn } = createHiddenUI();
  ...
  updateStats(hiddenListElement, statsBar, toggleBtn);
}
...
function handleToggle(
  ...
  chrome.storage.local.get([STORAGE_KEY], (res) => {
    ...
    chrome.storage.local.set({ [STORAGE_KEY]: ids }, () => {
      updateStats(hiddenList, stats, toggleBtn);
    });
  });
}
...
function createHiddenUI() {
  ...
  const lastBlock =
    forumBlocks[forumBlocks.length - 1] || document.body.lastElementChild;

  const container = document.createElement("div");
  container.id = "ivelt-hidden-vault-container";
  container.innerHTML = `
        <div class="vault-header" dir="rtl">
            <span style="color: white !important;">באהאלטענע אשכולות</span>
            <button type="button" class="vault-toggle-btn" id="ivelt-vault-toggle">ווייז באהאלטענע אשכולות</button>
        </div>
        <div id="ivelt-stats-bar" dir="rtl"></div>
        <ul id="ivelt-hidden-vault-list" class="topiclist topics" style="display: none !important;"></ul>
    `;

  lastBlock.after(container);

  const hiddenListElement = container.querySelector("#ivelt-hidden-vault-list");
  ...
  toggleBtn.addEventListener("click", () => {
    const isCurrentlyHidden =
      hiddenListElement.style.display === "none" ||
      hiddenListElement.style.getPropertyValue("display") === "none";
    hiddenListElement.style.setProperty(
      "display",
      isCurrentlyHidden ? "block" : "none",
      "important"
    );
    toggleBtn.innerText = isCurrentlyHidden
      ? "פארמאך רשימה"
      : "ווייז באהאלטענע אשכולות";
  });

  return { hiddenListElement, statsBar, toggleBtn };
}

function updateStats(hiddenList, statsBar, toggleBtn) {
  ...
  statsBar.innerHTML = `דער בלאט האט <b>${total}</b> באהאלטענע אשכולות (<b>${unread}</b> אומגעליינט, <b>${
    total - unread
  }</b> געליינט).`;
}
